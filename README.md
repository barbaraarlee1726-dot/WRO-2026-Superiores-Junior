...
The components used are essential to this robot; its design provides the right amount of space for each one. The EV3 motor delivers power and saves space thanks to the strategic position in which we’ve placed it. In that space, we’ve also installed a transmission system to increase the speed of the two wheels. In addition, it features a steering system that is both efficient and precise. The electronic components themselves are located on top. First is the heart of Timmy—the printed circuit board (PCB)—which essentially handles all the programming, including that of the motor and steering simultaneously, despite being a LEGO system. On either side of the board are several ultrasonic sensors that help prevent collisions with walls. The HuskyLens camera with the AI system is very useful, as it saves us a lot of programming work and detects the different pillars very well.  All these components were soldered or secured with screws because they need to be sturdy. One of our shortcomings is all the dead space the robot has. Dead spaces are unused areas, which means all the space we tried to save is wasted.
